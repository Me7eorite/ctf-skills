...
- Reuse the base image's non-root Web service user and conventional content
  directory when available, such as `www-data:/var/www/html` for Apache/PHP
  or the Tomcat image's `tomcat` account and application directory. Create
  `ctf` only when no suitable service user exists.
...
- Give the selected service user write access only to narrowly scoped runtime
  data directories.
- Do not use Compose `volumes`; copy application files and seed data into the
