...
- For Pwn container services, use native service artifacts such as `deploy/src/vuln.c`, `deploy/src/menu_service.cpp`, `deploy/src/Makefile`, or `deploy/src/bin/challenge`; do not use Web's Python `deploy/src/app.py` unless a Python wrapper is intentionally part of the design.
