...
The `web-console` capability defines the UX contracts that let operators run the entire CTF Skills platform from a browser. It pins the frontend technology stack (Vue 3 + Vite + TypeScript + Tailwind + shadcn-vue), the design system (semantic palette, typography, spacing, radius), the information architecture (seven top-level navigation groups), the "capability" display model exposed to users (challenge generator plus three coming-soon capabilities), the LLM provider configuration surface, the real-time event stream protocol, the documented routes and pages, and the supporting backend API surface.

It does NOT cover authentication, multi-tenancy, billing, dark mode, or mobile responsive polish. Other capabilities (`delivery-bundle`, `challenge-seed-management`, `re-target-platforms`, `module-architecture`, `hermes-execution-protocol`) remain unchanged. The pre-existing `dashboard-frontend` capability is left intact and is superseded only via subsequent changes, not by this sync.
