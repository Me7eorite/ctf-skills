...
- Add a bounded escalation path when solver repair cannot prove progress: regenerate the solver when safe, optionally require challenge regeneration when solver/target evidence proves the artifact is inconsistent, or fail with an explicit human-action reason.
...
No schema migration is required for the first slice unless the implementation chooses to persist a new durable blocked reason. Existing progress events, validation history, attempt summaries, report fields, and endpoint-derived fields should be preferred. Any new fields must be additive and bounded to the current attempt/result rows; list APIs must not scan unrelated execution histories.
