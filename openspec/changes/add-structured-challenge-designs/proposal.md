...
- Add `design_attempts` — one row per Hermes invocation against a
  ...
  path, error, and a fencing claim like `research_runs`.
- Add `challenge_designs` — one row per successful attempt, holding the
  ...
  schema).
- Add a deterministic **prompt wrapper** that materializes the
  `skills/design-challenges` skill template, the parent
  `generation_requests` topic + runtime constraints, the source
  `design_tasks` row, the cited `research_findings`/`research_sources`,
  and the category-specific references (`web-design.md` / `pwn-design.md`
  / `reverse-design.md` / `other-categories.md`).
- Add a **`DesignChallengeExecutor`** that claims a `queued` design
  task, runs Hermes once with the rendered prompt, captures stdout/log,
  and applies token-fenced terminal transitions.
...
- Drive `design_tasks` status: `queued → designing` on claim,
  `designing → designed` on validation success, `designing → failed` on
  any error, retry by inserting a new `queued`-status sibling row up to
  the parent request's `max_attempts`.
...
- **Does not** introduce a design worker pool, queue claim/lease, or
  ...
  operator action and runs in the request thread.
