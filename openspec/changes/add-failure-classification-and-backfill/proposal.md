...
- CLI `cli research backfill` 精简成两条用法：`--run-id <UUID> --dry-run`（审计/单条检查）和 `--all-recoverable --apply`（批处理历史积压）；**单条 apply 走 UI 不在 CLI 暴露**。
...
- `research-result-backfill`: 在 run 处于 `failed` 或 `running` 时，从 `hermes_log_path` 解析 Hermes 输出并把结果补落库；提供 UI 交互（preview → confirm → apply）与 CLI 应急/批处理通道。
