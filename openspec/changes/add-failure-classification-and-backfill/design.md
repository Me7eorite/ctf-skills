...
- CLI：保留 `challenge-factory research backfill --run-id <UUID> --dry-run`（一次审计输出）和 `--all-recoverable --apply`（一次性扫所有可救的 failed run）。**不暴露 CLI 单条 apply**——单条强制走 UI。
...
**选择**（apply 在同一事务取得 advisory lock 并 `SELECT ... FOR UPDATE` 后，按优先级递进。**digest 比对必须在 parse 之前**，让"操作员看到的预览字节 ↔ apply 真正消费的字节"窗口最小、且不浪费 CPU 在最终会被拒的字节上）：
...
- **apply 立刻对 safe-log 读出的字节计算 SHA-256，与 preview 提供值比较；不一致 → 409 `preview_stale`，不进入解析**
- `_extract_stdout_block` 命中 → 否则 422 `parse_failed:no_stdout_block`
- `_parse_research_output`（纯解析路径）成功 → 否则 422 `parse_failed:<reason>`
...
**理由**：每个失败都给操作员**精确诊断**，而不是一个泛化"失败"。digest 提早比对避免 TOCTOU 窗口被解析时间放大；`already_has_results` 避免重入 backfill 撞 UNIQUE。
...
6. **第六段**：CLI `challenge-factory research backfill --run-id ... --dry-run` 与 `--all-recoverable --apply`。最后落地，主要服务后续历史积压清理与审计需求。
