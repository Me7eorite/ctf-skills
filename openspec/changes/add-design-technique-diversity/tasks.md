...
- [ ] 1.1 Create `src/domain/design/technique_taxonomy.py` with: a `TechniqueFamily`
      lane enum per category (web: `auth|injection|server_side|client_side|upload|node_api|other`;
      pwn and re their own lanes), a `label`→lane keyword map, `resolve_family(finding) -> str`
      (stored value → keyword derivation → `other`), and
      `resolve_sub_technique(finding) -> str` (lowercase, trim, stop-word strip of `label`).
...
- [ ] 1.2 Unit-test `resolve_family`/`resolve_sub_technique`: stored valid value wins;
      ...
      with distinct sub-techniques; layered-encoding labels resolve to a stable family.
...
## 3. Two-axis diversity allocation

- [ ] 3.1 Rewrite `_findings_for_task` (`design_task_planning_service.py`) to take running
      `Counter[family]` and `Counter[sub_technique]`, prefer candidates violating neither
      axis, relax family→sub_technique→round-robin, and always return enough findings to
      preserve count.
...
- [ ] 3.4 Add `technique_quota` / `cooldown_window` knobs to `generation-profiles.json`
      and read them in the planner (sane defaults when absent).
...
- [ ] 5.3 Add `DesignTaskPlanningService.approve_plan(request_id)` (stamps
      `plan_reviewed_at` under the parent-row lock) and `regenerate_plan(request_id)`
      (reuses `replace_draft_or_archived_tasks`).
- [ ] 5.4 Add `DesignTaskPlanningService.regenerate_task(request_id, task_no)`: re-plan one
      slot with all *other* current drafts' families/sub-techniques in the avoid-set; same
      lock; reject if any sibling has left `draft`/`archived`.
- [ ] 5.5 Tests: unreviewed draft cannot be queued; approve then queue succeeds;
      regenerate-one removes a `subtechnique_duplicate` without disturbing siblings;
      regenerate blocked once any task is `queued`.
...
- [ ] 6.1 Add a plan-matrix view to the design-task dashboard (task_no / difficulty /
      ...
      `family_quota_exceeded` (warn) vs `subtechnique_duplicate` (error) distinctly.
- [ ] 6.2 Wire approve / regenerate-all / regenerate-one buttons to the service-backed HTTP
      endpoints (no direct DB writes from the dashboard).
- [ ] 6.3 Show the "research diversity insufficient — consider re-running research" hint when
      `subtechnique_duplicate` count exceeds a threshold.
