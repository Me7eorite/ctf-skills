...
- [ ] 1.2 Define the canonical closed class set as `timeout`, `service-readiness`, `prompt`, `contract`, and `solver`.
- [ ] 1.3 Define a small policy object for class-specific retry ceilings, repair routes, and stop conditions.
- [ ] 1.4 Thread the derived normalized failure class through runner progress messages and attempt summaries without adding a new durable source-of-truth table.
...
- [ ] 2.3 Ensure timeout and service-readiness failures do not consume a shared batch-wide retry budget.
...
- [ ] 4.1 Add tests for timeout, service-readiness, prompt, contract, and solver classification.
