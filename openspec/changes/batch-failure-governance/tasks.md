...
- [ ] 0.5 Expose `validation_failure_class`, normalized signature, and concise diagnostic summary in attempt list/detail; expose the selected route in repair context/progress summaries using the same latest-failed-validation derivation helper while keeping attempt-list derivation bounded to returned rows.
- [ ] 0.6 Add focused Milestone A tests for classification precedence, readiness-vs-solver EOF semantics, shared derivation, API/repair context exposure, repeated-signature stop after deterministic and Hermes repair, sibling continuation, and no `consecutive_infra` increment for validation failures.
