...
- Treat normalized `timeout` as a validation/validator-wrapper timeout only. Existing Hermes subprocess timeout failures, including `hermes_timeout` runner-phase failures and `HERMES_TIMEOUT` handling, remain outside the validation failure class set.
