...
- Add explicit solver/exp stability governance so Web/Pwn reference exploits use the live validation target, expose enough diagnostics for the next repair round, and do not silently fall back to brittle hardcoded hosts, ports, dependencies, or unbounded reads.
- Add a first-pass reference-solver quality gate for generated `exp.py`, especially Pwn payload construction, menu synchronization, libc/ROP assumptions, and reproducible diagnostic evidence before documentation is finalized.
