...
- Add a five-state machine for `build_attempts.status`: `queued`,
  `running`, `succeeded`, `failed`, `lost`. `lost` is a deliberate
  separate state for "shard or artifact directory disappeared", reached
  immediately on detection with no grace period.
...
- Add a `BuildOrchestrationService` that submits a batch of design tasks
  for building: in one transaction it inserts queued `build_attempts`
  rows, renders matrix-shaped shard JSON to `work/shards/pending/`, and
  moves the design tasks to `building`. A partial unique index on
...
- **BREAKING (dashboard chrome)**: the global header bar's `重新验证`,
  ...
  semantics.
- Extend the shard JSON schema with two forward-compatible optional
  top-level fields, `build_attempt_id` and `design_task_id`, plus a
...
- **Code**: add
  ...
  (status enum), `src/services/design_task_planning_service.py`
  ...
  button), and `prompts/shard_prompt.md` (a single sentence noting the
