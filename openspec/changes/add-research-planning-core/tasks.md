...
- [ ] 1.1 Add Alembic revision `0002_research_tables` (revises `0001_baseline`) defining the four enum types: `challenge_category (web|pwn|re)`, `generation_request_status (draft|researching|researched|failed)`, `research_run_status (queued|running|completed|failed)`, `research_finding_kind (technique|variant|scenario|prerequisite)`. The `challenge_category` values MUST exactly match `core.queue.SUPPORTED_CATEGORIES`; add a comment in the migration file pointing at that constant so future divergence is caught at review time.
- [ ] 1.2 Add `generation_requests` table: `id uuid pk`, `category challenge_category not null`, `topic text not null`, `target_count int not null check (target_count > 0)`, `difficulty_distribution jsonb not null`, `runtime_constraints jsonb not null default '{}'::jsonb`, `status generation_request_status not null default 'draft'`, `created_at timestamptz not null default now()`, `updated_at timestamptz not null default now()`. Index on `(category, status)` for category-filtered listings.
...
- [ ] 1.7 Write reversible `downgrade()` that drops tables in reverse dependency order (join → findings → sources → runs → requests) and drops the four enum types (`research_finding_kind`, `research_run_status`, `generation_request_status`, `challenge_category`).
...
- [ ] 2.1 Create `src/domain/research.py` with `@dataclass(frozen=True)` DTOs: `GenerationRequest` (with `category: str`), `ResearchRun`, `ResearchSource`, `ResearchFinding`, plus enums `GenerationRequestStatus`, `ResearchRunStatus`, `ResearchFindingKind`, and the allowed `DIFFICULTY_LABELS = ("easy","medium","hard","expert")`. Re-export `core.queue.SUPPORTED_CATEGORIES` as the canonical category set; do NOT duplicate the literal values.
...
- [ ] 2.3 Add `validate_category(category)` raising `ResearchValidationError` when `category` is empty/None or not in `SUPPORTED_CATEGORIES`; message lists the allowed set.
...
- [ ] 4.2 Create `src/persistence/repositories/research.py::ResearchRepository(session)` with: `create_generation_request(*, category, topic, target_count, difficulty_distribution, runtime_constraints) -> GenerationRequest` (validates category + distribution + target_count before insert), `get_generation_request(id) -> GenerationRequest | None`, `list_generation_requests(*, category=None, status=None) -> list[GenerationRequest]` (both filters optional, AND-combined), `create_research_run(generation_request_id) -> ResearchRun`, `mark_run_running(run_id, started_at) -> ResearchRun`, `mark_run_completed(run_id, finished_at, log_path) -> ResearchRun`, `mark_run_failed(run_id, finished_at, error, log_path) -> ResearchRun`, `add_source(run_id, *, url, title, summary, content_hash, fetched_at, raw_text_path=None) -> ResearchSource`, `create_finding(run_id, *, kind, label, summary, source_ids) -> ResearchFinding`, `get_run(id) -> ResearchRun | None`, `list_sources(run_id) -> list[ResearchSource]`, `list_findings(run_id) -> list[ResearchFinding]`. The repository SHALL call into `domain.research.validate_*` before any insert.
...
- [ ] 4.5 Add `tests/app/test_research_repository.py` marked `@pytest.mark.postgres`: round-trip insert and list, list-by-category filter returns only matching category, unknown category rejected, cascade-delete from generation request through findings, finding-without-source rejection, finding-with-unknown-source-id integrity error, distribution validation propagation.
...
- [ ] 7.1 Add `prompts/research_prompt.md` describing the input fields (category, topic, target_count, difficulty_distribution, seed_urls, runtime_constraints) and the output contract: a single JSON object with `sources[]` and `findings[]`. Document the schema of each entry and require every `findings[i].source_indices` to be a non-empty list of valid integer indices into `sources[]`. Instruct the Agent to keep all findings within the declared category (web / pwn / re) and to refuse cross-category material rather than mixing it in silently. Include a worked example matching the rendered category.
- [ ] 7.2 Add `src/hermes/prompt.py::render_research_prompt(generation_request, seed_urls)` returning the rendered text; the rendered output MUST include the category at the top of the prompt body (not buried in the JSON spec) so the Agent reads it before anything else. Reuse the existing prompt-rendering style.
...
- [ ] 7.4 Add `tests/app/test_research_prompt.py` asserting the rendered output contains the declared category at a prominent position, the JSON schema description, the source-index constraint phrase, the "keep findings within category" instruction, and the worked example. Parametrize over `web | pwn | re`.
...
- [ ] 9.2 Subcommand `research submit --category {web,pwn,re} --topic STR --count INT --difficulty LABEL:N[,LABEL:N...] [--seed-url URL]... [--timeout INT]`: `--category` is required and validated via argparse `choices=`; creates a `generation_requests` row via the repository, transitions it to `researching`, spawns a `ResearchRunner.execute(...)` synchronously, prints the run id, category, and final status to stdout, and exits 0 on `completed` / 1 on `failed`.
- [ ] 9.3 Subcommand `research show <generation_request_id>`: prints request fields including category, run status, source count, finding count grouped by `kind`, and the latest run's log path. Unknown id prints a clear error and exits 2.
- [ ] 9.4 Subcommand `research list [--category {web,pwn,re}] [--status STATUS]`: prints id, category, topic, target_count, status, created_at, one per line. Both filters are optional and AND-combined.
- [ ] 9.5 Add `tests/app/test_research_cli.py` covering `--category` required + invalid value rejected by argparse, `--difficulty` parsing (valid + invalid), `submit` happy path with a stubbed runner, `submit` exit 1 on failed run, `show` and `list` rendering, `list --category` filter.

## 10. Web read endpoints

- [ ] 10.1 Add `GET /api/research/requests?category=...&status=...` to `src/web/server.py` returning a JSON array of generation requests. Both query params optional and AND-combined. Unknown `category` value returns 400 with a message naming the allowed set.
...
- [ ] 12.1 Add `tests/app/test_research_alembic.py` marked `@pytest.mark.postgres` mirroring `test_alembic_migrations.py`: upgrade to `0002_research_tables`, assert the five tables exist via `inspect`, assert the four enums exist with the expected labels (including `challenge_category` having exactly `{web, pwn, re}`), assert `challenge_category` labels match `core.queue.SUPPORTED_CATEGORIES`, downgrade to `0001_baseline`, assert only `alembic_version` remains and no `challenge_category` type is left behind.
...
- [ ] 14.5 Run the dry-run smoke: `challenge-factory research submit --category web --topic "SQL injection sample" --count 4 --difficulty easy:2,medium:2 --seed-url https://example.com/sqli --timeout 60`. Either: a real Hermes runs end-to-end against the dev DB OR a stub Hermes binary returns canned JSON. Confirm a `research_runs` row appears with the expected status and that the parent `generation_requests` row has `category="web"`.
