...
For `category = pwn`, structured Design SHALL declare a pwn primitive contract as part of the validated design/build-contract payload. The contract SHALL include `primitive_id`, `primitive_version`, `expected_source_features`, `disqualifiers_considered`, `evidence_requirements`, and `offset_policy`. The Design stage SHALL NOT claim that generated source or compiled binary evidence has already been observed.
...
- **WHEN** a pwn Design output includes semantic proof that depends on generated source, compiled layout, or debugger output
...
- **WHEN** a pwn Design output declares `primitive_id = stack_overflow_basic`
