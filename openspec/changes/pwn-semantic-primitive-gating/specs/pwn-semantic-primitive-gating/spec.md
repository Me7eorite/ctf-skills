...
- **AND** only an accepted `observation_review_decision` with the configured pwn-semantic scope may provide effective acceptance for that inconclusive supported-primitive case
...
- **AND** there is no accepted `observation_review_decision` with the configured pwn-semantic scope bound to the current ArtifactObservation for the same BuildAttempt, DesignEvidence/build-contract version, canonical `contract_sha256`, and artifact manifest hash
