...
- [ ] 1.4 Add diagnostic codes for `pwn_primitive_not_realized`, `pwn_primitive_disqualified`, `pwn_primitive_evidence_missing`, `pwn_primitive_unsupported`, `pwn_challenge_escape`, `pwn_primitive_stale_evidence`, and `pwn_primitive_inconclusive`.
- [ ] 1.5 Define deterministic diagnostic precedence: unsupported id, stale/unbound artifact evidence (`pwn_primitive_stale_evidence`), challenge escape, primitive-specific disqualifier hit, missing required evidence, non-equivalent primitive realization, inconclusive evidence, then generic not-realized fallback.
...
- [ ] 2.1a Keep `build_contract.pwn_primitive_contract` category-scoped: require it only for governed pwn Design/BuildAttempt paths, reject it for non-pwn contracts unless a future capability explicitly allows it, and preserve legacy unguided pwn shards without the contract.
...
- [ ] 2.2a Treat contract `evidence_requirements`, `offset_policy`, and primitive version fields as host-rendered primitive-definition snapshots/references; reject contracts that try to override host-owned primitive semantics.
...
- [ ] 3.1 Implement a host-owned pre-build source semantic gate for governed pwn BuildAttempts after implementation source exists and before image build, integrated with the existing pre-build contract gate lifecycle.
...
- [ ] 3.6 Persist semantic-audit outcome and primitive fingerprint material in ArtifactObservation `contract_checks` / `fingerprints`, bound to BuildAttempt, DesignEvidence/build-contract version, canonical `contract_sha256`, ArtifactObservation `artifact_manifest_sha256`, and current host validation observation.
...
- [ ] 3.8 Add pwn-semantic scoped observation-review policy, for example scope `pwn-semantic-primitive`, for `pwn_primitive_inconclusive` supported-primitive cases only; do not reuse the production-publication scope as semantic acceptance.
- [ ] 3.8a Prevent observation review acceptance for unsupported, challenge-escape, disqualified, not-realized, or stale observations.
...
- [ ] 4.8 Add tests proving governed inconclusive evidence fails closed unless an accepted pwn-semantic scoped `observation_review_decision` is bound to the current ArtifactObservation for the same BuildAttempt, DesignEvidence/build-contract version, canonical `contract_sha256`, and ArtifactObservation `artifact_manifest_sha256`.
- [ ] 4.8a Add tests proving observation review cannot override unsupported, challenge-escape, disqualified, not-realized, or stale-artifact evidence observations, and that `production-publication` review alone does not accept pwn semantic inconclusive evidence.
