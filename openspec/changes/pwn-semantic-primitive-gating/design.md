...
- [Risk] A valid but unusual exploit may be rejected. -> Mitigation: route unsupported primitive contracts to human review or add a new primitive definition with tests before accepting it in governed automation.
...
- [Risk] Primitive-gating can overfit to signature checks and miss challenge escapes. -> Mitigation: explicitly audit fixed secret checks, plaintext flag exposure, debug/backdoor paths, and stronger-than-declared primitive supersets as separate escape diagnostics.
...
- Should primitive supersets such as arbitrary write in a declared ret2win challenge fail automatically, or route to review based on difficulty policy?
