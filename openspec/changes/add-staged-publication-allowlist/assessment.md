...
## 第 31 轮：Task 5 + 4.2/4.3 + 1.10 实施结果

实施范围（一次性 PR）：
- **Task 5.0** 新增 `paths.locks_root` 与 `paths.build_publisher_locks`，
  `initialize()` 创建；publisher preflight 在 `fcntl` 不可用时以
  `WorkspacePublishError(phase="contract")` 拒绝，明确非 POSIX 不支持。
- **Task 5.1** `_acquire_publisher_locks(category, claimed_ids)` 按 (category,
  claimed_id) 排序、digest 文件名（`sha256(category\0claimed_id).hexdigest()`），
  `fcntl.flock(LOCK_EX | LOCK_NB)` + 默认 30 秒超时（`BUILD_PUBLISH_LOCK_TIMEOUT_SECONDS`
  正整数 override，非法值 preflight fail）。Lock 文件不嵌入主机路径。
- **Task 5.2** publisher 现在先把所有 claimed id 全 staging 到 temp sibling，
  全部成功后才进入 canonical mutation 循环；commit 前调用
  `_verify_same_filesystem` 对比 `canonical / quarantine / workspace.output`
  的 `st_dev`，跨设备直接 `phase="commit"` 失败。
- **Task 5.3** `_write_publish_journal` 现在带 entries/category，
  `_write_atomic_json(fsync=True)` 在 temp file fsync + 目录 fsync 后 rename；
  phase 变化（stage → manifest → committed → rollback）都会 fsync 持久化。
- **Task 5.4** quarantine 路径冲突仍走 `repair-<uuid>` 后缀；新流程下批量 rollback
  正确处理。
- **Task 5.5** 失败时 `_rollback_published_batch` 反向 unwind 已 rename 的
  canonical（包括从 quarantine 还原 predecessor），并 `shutil.rmtree` 全部
  staged temps；journal 标记为 `rollback`。
- **Task 5.6** 新增 `_bootstrap_recover_journal`：每次 publish 前自动调用；
  发现 in-flight journal 时使用同一锁、按 phase 决定是 finalize 还是 rollback；
  recovery 本身不消耗 generation；可重入幂等。
- **Task 5.7** journal phase 为 `committed` 时 recovery 把
  `state/highest-committed-generation.json` 推进到 journal 的 generation，
  随后删除 journal；canonical / manifest 不回滚，re-run 是 no-op。
- **Task 5.8** lock 文件由 publisher 创建后从不删除；orphan 无害，注释在
  spec/Task 5.8 中已说明。
- **Task 4.2/4.3** rehash canonical 在锁内进行；manifest 写失败统一进入
  `_write_publish_journal(phase="rollback")` 并恢复 predecessor；crash window
  由 `committed` phase journal + bootstrap recovery 覆盖。
- **Task 1.10** `_cross_check_resume_targets` 在 contract phase 检查
  `resume_output_targets` 与 staging basename 是否一致；缺失/不一致以
  `WorkspacePublishError(phase="contract")` 失败，引用 recorded/observed。

实施中发现并附带处理的小问题：
- `_write_atomic_json` 加 `fsync=True` 后破坏了一个 monkeypatch 测试签名；
  测试更新为 `def fail_manifest(path, payload, **kwargs)` 透传 kwargs。
- `tests/app/test_runner_resume.py::_Paths` duck-type 缺少
  `build_publisher_locks` 等属性；补齐 `locks_root`、`build_publisher_locks`、
  `executions` 三个 property；`initialize()` 仅创建 `build_publisher_locks`，
  保留 dry-run 不创建 `executions` 的旧断言语义。
- publisher 增加 `_write_terminal_marker(state/publish-status.json)`：所有
  succeeded / noop / failed 终态都写 marker，为 Task 6 retention sweep 提供
  host-owned timestamp/status 信号。

验证：
- `uv run pytest tests/app -q` → **727 passed**, 1 warning, 5 subtests passed
- `uv run ruff check src/hermes/build_publisher.py src/core/paths.py
  tests/app/test_build_publisher.py tests/app/test_runner_resume.py`
  → All checks passed
- `openspec validate add-staged-publication-allowlist --strict` → valid

下一步：Task 6 retention sweep（throttle + terminal marker + active-journal skip）
可直接基于本轮新写入的 `state/publish-status.json` 与 `state/publish-journal.json`
做扫描。

## 第 32 轮：Task 6 retention sweep + Task 7 runner integration

实施范围：
- **Task 6 retention sweep**：`_run_retention_sweep(paths)` 在所有 publish 路径
  （succeeded / noop / failed）后调用；`_sweep_retention_roots(paths)`
  扫描 `paths.executions/*` workspace，按 `state/publish-status.json` 终态
  时间戳判断 retention root，结合 7 天 / 20 root 双约束清理 `quarantine/ output/ logs/`。
  跳过 in-flight journal 与无法非阻塞取锁的 workspace；
  per-process throttle 默认 60 秒（`BUILD_PUBLISH_SWEEP_INTERVAL_SECONDS`
  正整数 override），被压制调用置 `pending`，下一次必跑。错误降级为 logger
  warning，不影响 publish 结果。
- **Task 7.10 runner 消费 noop**：runner 在 validation repair loop 中捕获
  `publish_workspace_output` 的返回值；`outcome == "noop"` 时跳出 loop、
  不再 rerun validator、不更新进度。
- **Task 7.4/7.6/7.8 execution_mode + resume_output_targets**：
  `materialize_resume_outputs` 改为返回 `{claimed_id: workspace_relative_path}`
  映射；runner 在 materialize 后把映射写入 `input/manifest.json::resume_output_targets`。
  runner 解析 execution_mode：显式 `clean` 跳过 materialize 与 carry-forward；
  显式 `resume` 必须带安全 basename；未声明的 legacy payload 走 "implicit"
  shim（继续 materialize），由 Task 9 在测试套件迁移时收紧。
- **Task 7.1–7.3, 7.9** 已在早期实施中完成；本轮重新核对，runner 现在把
  publisher 失败映射为 `status=failed, failure_type=infrastructure`、保留
  phase/id/path 诊断，与 BuildReconciler `state=failed` 对齐。

实施中发现并处理：
- 旧测试 `test_failed_validation_is_fed_back_to_hermes_and_retried` 在
  `workspace.output` 顶层写 marker，但 publisher 仅哈希 claimed 目录内文件，
  导致 staging-hash 不变、publisher 正确返回 `noop`，runner 跳出 repair loop，
  validator 第二次未触发。测试已更新为把 marker 写入
  `workspace.output/challenges/web/web-0001-demo/` 内（claimed 目录），
  使两次 publish 哈希不同、validator 仍被调用两次。
- runner 对 legacy payload（无 `execution_mode` 且无 `resume_from_shard_basename`）
  保留 "implicit" 行为：继续 materialize、写 resume_output_targets。publisher
  仍按 spec 把同一 payload 归类为 `clean`，但 publisher 的 cross-check 在
  `resume_output_targets` 为空时短路、不会因此误报。
- Task 7.5（prompt 渲染目标路径硬约束）与 Task 7.7 后半段（clean 模式跳过 prior-shard
  carry-forward）依赖 prompt 模板更新与 Task 7A 的 `BuildOrchestrationService.clean_rebuild`，
  本轮保留未勾选；其他 7.x 已全部勾选并通过测试。

验证：
- `uv run pytest tests/app -q` → **727 passed**, 1 warning, 5 subtests passed
- `uv run ruff check src/hermes/runner.py src/hermes/workspace.py
  src/hermes/build_publisher.py tests/app/test_runner_resume.py` → All checks passed
- `openspec validate add-staged-publication-allowlist --strict` → valid

下一步：Task 7A clean rebuild 全栈（service / API / UI / alembic 迁移），
然后 Task 8 spec migration、Task 9 测试套件、Task 10 cleanup。
...
第 26–29 轮针对 Task 1–4 实施收尾后再做一轮"未实施 tasks 5–10 是否还有题案矛盾"
的独立审视，识别 5 项：

- **G7（高）runner 没有任务消费 PublishResult.outcome**：Task 4.4 已实现 publisher
  返回 `noop`，spec 也有 "Runner observes noop and exits the repair loop" 场景，但
  Task 7（runner integration）没有任何一条要求 runner 读取 `outcome` 并据此跳出
  validation repair loop。当前 runner.py 完全不查看 PublishResult。整改：tasks 新增
  **7.10** "Read PublishResult.outcome; on `noop` exit the validation-repair loop
  without rerunning the validator and without advancing progress percent."
- **G8（中-高）resume target 与 staging 不一致缺少实现任务**：spec "Resume target
  binding disagrees with staging" 要求 publisher 在 contract phase 拒绝；但
  `_collect_candidates` 只按 claimed_ids 集合扫描，没有与 contract.resume_output_targets
  做基名级别的交叉校验。整改：tasks 新增 **1.10** "Cross-check `_collect_candidates`
  result against `contract.resume_output_targets`; mismatched/missing target SHALL
  fail in `contract` phase before canonical mutation, reporting both recorded
  target and observed path."
- **G9（中）runner pre-check 与 publisher noop 双重短路缺乏优先级说明**：runner.py
  已有 `pre_signature == post_signature → break` 的 fast-path，与 publisher noop
  在功能上重叠。spec 的 noop scenario 又用了"runner's no-changes detector aborted
  the repair before Hermes edited anything"做例子——若 pre-check 已 break，publisher
  根本不会被调用，该例子误导。整改：
  - design 新增 **Decision 13**：runner output-signature pre-check 是性能 fast-path，
    可被未来取消；publisher 的 staging-hash noop 是 fail-closed 的最终保证，
    任何时候都必须存在；两层短路同时存在不冲突，但 spec 不依赖 pre-check 存在。
  - 删除 spec noop scenario 里"the runner's no-changes detector aborted the
    repair before Hermes edited anything"的误导例子，改为"Hermes 完成了 repair
    但产出与上一次 byte-identical（例如只改了 logs）"。
- **G10（小）`promote_claimed_outputs` 删除与 `WorkspacePromotionError` 保留未区分**：
  Task 10 写"只允许 import `hermes.build_publisher.publish_workspace_output`"，但
  `WorkspacePromotionError` 是 `WorkspacePublishError` 的基类，runner/publisher 仍要
  import 该异常名。整改：spec migration 文段和 Task 10.1/10.2 显式说明删除的是
  函数 + 同名 deprecation stub；异常类作为基类保留；CI grep guard 只匹配函数名
  （例如 `\bpromote_claimed_outputs\(`），不可匹配 `WorkspacePromotionError`。
- **G11（小）Task 4 状态语义模糊**：4.2/4.3 已说明依赖 Task 5，但 Task 4 标题本身
  没标"partial"。整改：在 tasks.md Task 4 标题后加 "(4.1/4.4 land in this PR;
  4.2/4.3 depend on Task 5's locks & journal)"，避免读者把 Task 4 当作可独立勾完。

## 总体结论（v7）

第 30 轮整改后，本题案在进入 tasks 5–10 实施前的新增不变量：

- runner 必须显式消费 `PublishResult.outcome`；noop 不再依赖 runner 的 fast-path
  做语义判断，仅作为性能优化共存；
- publisher 在 contract phase 内交叉校验 staging 与 `resume_output_targets`，
  resume target 不再只用作 prompt 装饰，而是 fail-closed 的源；
- `promote_claimed_outputs` 函数及其 stub 与 `WorkspacePromotionError` 异常类
  在归档时区别对待；CI guard 仅约束函数符号；
- Task 4 的"部分完成"状态在 tasks.md 显式标注，避免与 Task 5 解耦时读者误判。
